...
function ReceiptModal({client, month, year, amount, date, onClose}) {
  ...
  const getCanvas = async () => {
    ...
    if(window.html2canvas) {
      return window.html2canvas(el, {scale:3, backgroundColor:'#fff', useCORS:true});
    }
    return new Promise((res,rej) => {
      const s = document.createElement('script');
      s.src = 'https://cdnjs.cloudflare.com/ajax/libs/html2canvas/1.4.1/html2canvas.min.js';
      s.onload = () => window.html2canvas(el,{scale:3,backgroundColor:'#fff',useCORS:true}).then(res);
      s.onerror = rej;
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      a.download = 'makbuz_'+client.name+'_'+donem.replace('/','_')+'.png';
  ...
  const handleWhatsApp = async () => {
    ...
    try {
      ...
      canvas.toBlob(async (blob) => {
        if(navigator.share && navigator.canShare && navigator.canShare({files:[new File([blob],'makbuz.png',{type:'image/png'})]})) {
          await navigator.share({files:[new File([blob],'makbuz.png',{type:'image/png'})], title:'Makbuz'});
        } else {
          // Fallback: PNG indir + WhatsApp web aç
          const a = document.createElement('a');
          a.download = 'makbuz.png';
  ...
  return(
    <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,.85)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:2000,backdropFilter:'blur(4px)',padding:16}}>
      <div style={{background:'#fff',borderRadius:16,width:'100%',maxWidth:420,boxShadow:'0 32px 80px rgba(0,0,0,.6)',overflow:'hidden'}}>
        ...
        <div id="receipt-preview" style={{padding:16,background:'#fff'}}>
        ...
        <div style={{padding:16,background:'#f8fafc',display:'flex',flexDirection:'column',gap:10}}>
          <button onClick={handleWhatsApp} disabled={saving}
            style={{background:'#25d366',border:'none',borderRadius:10,padding:'13px',color:'#fff',fontWeight:700,fontSize:15,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',gap:10,opacity:saving?0.7:1}}>
            ...
            {saving ? 'Hazırlanıyor...' : 'WhatsApp ile Gönder (Resim)'}
          </button>
          <div style={{display:'flex',gap:10}}>
            <button onClick={handleSave} disabled={saving}
              style={{flex:1,background:'#1a2744',border:'none',borderRadius:10,padding:'11px',color:'#fff',fontWeight:700,fontSize:14,cursor:'pointer',opacity:saving?0.7:1}}>
              {saving ? '...' : '⬇ PNG Kaydet'}
            </button>
            <button onClick={onClose}
              style={{flex:1,background:'#fff',border:'1px solid #e2e8f0',borderRadius:10,padding:'11px',color:'#475569',fontWeight:600,fontSize:14,cursor:'pointer'}}>
              Kapat
            </button>
          </div>
        </div>
